...
// кол-во кластеров = 6
// общая асимптотика ~= O(6^n), общий расход памяти (в абсолютном пике) = 141 + 21n
...
struct cluster_masks{
    ...
    // асимптотика = O(k * n) ~= O(n), где k - кол-во повторений взятия след маски, расход = 6 (от check_mask())
    void get_next(){
        ...
        if (!check_mask()) get_next(); // если маска не подходит (см. описание check_next()), то берем следующую маску, возможная асимптотика = O(n)
    }
    ...
    // общая асимптотика = O(n), расход = 6
    bool check_mask(){
        std :: vector<bool> encounter(6, false);    // встречался ли номер k-ого кластера в маске, расход = 6 * 1 = 6
        for (int i = 0; i < curr_mask.size(); i++){ // проходимся по всей маске, O(n)
            encounter[curr_mask[i]] = true;
        }
        for (int i = 0; i < 6; i++){                // проверяем, все ли номера кластеров встретились, O(6) ~= O(1)
            if (!encounter[i]) return false;        // если не встретился номер - маска не подходит
        }
        return true;
    }
};
...
// асимптотика ~= O(6^n) , расход = 2n + 5 + 16 + 4n + 8n = 21 + 14n
std :: vector<std :: vector<double>> clusterize(std :: vector<double> &arr){
    ...
    cluster_masks masks(arr.size());        // 2n + 5
    ...
    while(!masks.done){                     // так как рассмотрим абсолютно все маски -> рассмотрим все числа шестеричной системы от 0 до 5 5 5 ... 5 -> 6^n вариантов, O(6^n)
        current_score = get_score(masks.curr_mask, arr); // O(n)
