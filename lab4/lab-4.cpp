...
// кол-во кластеров = 6
// общая асимптотика = O(), общий расход памяти (в абсолютном пике) = 141 + 21n
...
struct cluster_masks{
    ...
    void get_next(){
        ...
        if (!check_mask()) get_next(); // если маска не подходит (см. описание get_next()), то берем следующую маску, возможная асимптотика = O(n)
    }
